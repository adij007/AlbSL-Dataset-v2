...
This document wires **your existing repo** (MediaPipe extraction in `data/`, CSV/NPZ layouts) to the new pipeline: **consolidation → `train_albsl.py` → `albsl_model_final/` → `albsl_app_v3.py`**.
...
**Relation to the rest of the project**: video CSVs use the same 63 hand columns as `albsl_fusion.data.VIDEO_63` (and `data/csv/videos/NN_Letter.csv`). NPZ files follow `extract_keypoints_v2` (`xyz` / `xyz_right` per frame). This keeps consolidation aligned with the fusion / CSV ecosystem, not a separate ad-hoc format.

## 3) Train (`train_albsl.py`)
...
- **Logs**: `training_log.jsonl` (per epoch), `convergence_log.jsonl` (per round, checkpoint path).

```powershell
# GPU + 4-bit (if available)
python train_albsl.py --data-dir albsl_dataset_v2 --4bit

# CPU or 4bit broken on your machine
python train_albsl.py --data-dir albsl_dataset_v2 --no-4bit
```

**Outputs**:

- `checkpoints/state.pt` (last) and `checkpoints/round_*.pt`
- `albsl_model_final/model_full.pt` — `state_dict` + `lmap` (unless `--no-export`)
- `albsl_model_final/label_map.json`, optional `albsl_landmark.onnx`

## 4) `albsl_model_final` and `albsl_landmarks.json` for v3

- **`albsl_app_v3.py`**: loads `albsl_model_final/model_full.pt` for the **63-D** model. It **still** reads `albsl_landmarks.json` (same file name and `letter → 21×3` layout as the older letter collector) and uses it **only** when softmax confidence is below `--min-conf` (template + distance in canonical 21×3).  
...
```powershell
python albsl_app_v3.py --landmarks-json albsl_landmarks.json --model albsl_model_final/model_full.pt --camera 0
```

Press **Q** in the window to quit.

## 6) Knobs (edit constants in `train_albsl.py` if needed)
...
1. [ ] `python consolidate_data.py`  
2. [ ] Inspect `albsl_dataset_v2/consolidation_report.md`  
3. [ ] `python train_albsl.py --data-dir albsl_dataset_v2`  
4. [ ] `albsl_model_final/model_full.pt` exists  
5. [ ] `albsl_landmarks.json` present (fallback)  
6. [ ] `python albsl_app_v3.py`

---

For **Intel-only** extraction and legacy apps, keep using `extract_keypoints_v2.py`, `albsl_app_v2.py` (MLP + diagnose/train/live) as in the main README you maintain for that track.
